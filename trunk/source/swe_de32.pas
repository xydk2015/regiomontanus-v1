...
function swe_calc(tjd_et       : double;    {Julian day, Ephemeris Time}
...
function swe_csnorm(p: Longint): Longint; stdcall; far; external 'swedll32.dll' name '_swe_csnorm@4';

function swe_csroundsec(x: Longint): Longint; stdcall; far; external 'swedll32.dll' name '_swe_csroundsec@4';
...
function swe_day_of_week(var tjd: double): integer; stdcall; far; external 'swedll32.dll' name '_swe_day_of_week@8';

function swe_degnorm(x: double): double; stdcall; far; external 'swedll32.dll' name '_swe_degnorm@8';

function swe_deltat(tjd  : double): double; stdcall; far; external 'swedll32.dll' name '_swe_deltat@8';
...
function swe_d2l(x: double): Longint; stdcall; far; external 'swedll32.dll' name '_swe_d2l@8';
...
function swe_fixstar_ut(star   : pchar;     {star name}
                   ...
                   var xx      : double;    {first of 6 doubles}
                   sErr        : pchar      {Error-String}
): Longint; stdcall; far; external 'swedll32.dll' name '_swe_fixstar_ut@24';

function swe_get_planet_name(ipl : integer;
                   pname      : PChar
): PChar; stdcall; far; external 'swedll32.dll' name '_swe_get_planet_name@8';

function swe_get_tid_acc(t_acc: double): double; stdcall; far; external 'swedll32.dll' name '_swe_get_tid_acc@0';
...
procedure swe_set_jpl_file(fname: pchar); stdcall; far; external 'swedll32.dll' name '_swe_set_jpl_file@4';

procedure swe_set_tid_acc(t_acc: double); stdcall; far; external 'swedll32.dll' name '_swe_set_tid_acc@8';
...
function swe_sidtime(tjdut: double): double; stdcall; far; external 'swedll32.dll' name '_swe_sidtime@8';
...
function swe_get_ayanamsa(tjd_et: double): double; stdcall; far; external 'swedll32.dll' name '_swe_get_ayanamsa@8';

function swe_get_ayanamsa_ut(tjd_ut: double): double; stdcall; far; external 'swedll32.dll' name '_swe_get_ayanamsa_ut@8';
...
function swe_sol_eclipse_where(tjd_ut  : double;
                            ...
                            var geopos : double;  {longitude, latitude, height above sea}
                            var attr   : double;  {first of 20 doubles}
                            serr       : pchar
): longint; stdcall; far; external 'swedll32.dll' name '_swe_sol_eclipse_where@24';
...
function swe_sol_eclipse_how(tjd_ut    : double;  {time, Jul. day UT}
                            ...
                            var attr   : double;  {first of 20 doubles}
                            serr       : pchar
): longint; stdcall; far; external 'swedll32.dll' name '_swe_sol_eclipse_how@24';
...
function swe_sol_eclipse_when_loc(tjd  : double;  {start date for search, Jul. day UT}
                            ...
                            var attr   : double;  {first of 20 doubles}
                            backward   : boolean; {TRUE, if backward search}
                            serr       : pchar
): longint; stdcall; far; external 'swedll32.dll' name '_swe_sol_eclipse_when_loc@32';
...
function swe_sol_eclipse_when_glob(tjd  : double;  {start date for search, Jul. day UT}
                             ...
                             backward   : boolean; {TRUE, if backward search}
                             serr       : pchar
): longint; stdcall; far; external 'swedll32.dll' name '_swe_sol_eclipse_when_glob@28';
...
function swe_lun_eclipse_how(tjd_ut     : double;  {time, Jul. day UT}
                             ...
                             var attr   : double;  {first of 20 doubles}
                             serr       : pchar
): longint; stdcall; far; external 'swedll32.dll' name '_swe_lun_eclipse_how@24';
...
function swe_lun_eclipse_when(tjd       : double;  {start date for search, Jul. day UT}
                             ...
                             backward   : boolean; {TRUE, if backward search}
                             serr       : pchar
): longint; stdcall; far; external 'swedll32.dll' name '_swe_lun_eclipse_when@28';
...
procedure swe_azalt(tjd_ut  : double;  {UT}
                 ...
                 atpress    : double;  {atmospheric pressure in mbar (hPa)}
                 attemp     : double;  {atmospheric temperature in degrees Celsius}
                 var xin    : double;  {ecliptical or equatorial coordinates}
                 var xaz    : double   {azimuth, true altitude, apparent altitude}
...
procedure swe_azalt_rev(tjd_ut  : double;  {UT}
                     ...
                     var xout   : double   {ecliptical or equatorial coordinates: x,y,z}
...
function swe_rise_trans(tjd_ut : double;  {Search after this time (UT)}
                    ...
                    var tret   : double;  {first of 10 doubles}
                    serr       : pchar
): longint; stdcall; far; external 'swedll32.dll' name '_swe_rise_trans@52';
...
function swe_nod_aps_ut(tjd_ut : double; {Julian day, Universal Time}
                  ...
                  var xaphe : double;    {First of 6 doubles for aphelion}
                  serr      : pchar
): longint; stdcall; far; external 'swedll32.dll' name '_swe_nod_aps_ut@40';
...
const
   ...
   // Csomópontok
   SE_MEAN_NODE{: integer{}       = 10; // Felszálló holdcsomó - ÁTLAGOS
   SE_TRUE_NODE{: integer{}       = 11; // Felszálló holdcsomó - VALÓDI
   SE_MEAN_APOG{: integer{}       = 12; // Lilith - DarkMoon ?!
   ...
   // 55 - Vulcan ; 56 - Selena ; 57 - Proserpina ; 58 - Waldemath

   SE_ERIS = SE_AST_OFFSET + 136199; // Eris planéta
...
{Flag bits for parameter iflag in function swe_calc()
 ...
 If not only coordinates, but also speed values are required, use iflag = SEFLG_SPEED.}
   ...
   SE_MAX_STNAME : Longint = 20; {maximum size of fixstar name;
                                   the parameter star in swe_fixstar must allow twice this space for the returned star name.}

   SEHOUSE_SYSTEM: array[0..9] of Char = ('P', 'K', 'O', 'R', 'C', 'E', 'V', 'X', 'H', 'T');
{
    'P'          Placidus
    'K'          Koch
    'O'          Porphyrius
...
function GetHouseSystemName(AHouseSystem: string) : string;
...
begin
  Result := '';
  case AHouseSystem[1] of
  'P' : Result := 'Placidus';
  'K' : Result := 'Koch';
  'O' : Result := 'Porphyrius';
  'R' : Result := 'Regiomontanus';
  'C' : Result := 'Campanus';
  'A' : Result := 'Egyenlõ';
  end;
end;
